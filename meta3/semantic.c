...
void local_dfs(No*current, table *last_table) {
	...
	if(strcmp(current->nome, "FuncDecl") == 0) {
		last_table = last_table->next;
		
		if(current->filho->irmao->filho != NULL) {						/* Entrar directo para o primeiro filho de FuncBody se esta tiver um filho */
...
void function_dfs(No*current, table *c_table) {							/* Estamos no primeiro filho do FuncBody, ou seja, ver os irmãos se há VarDecls */
	if(current == NULL)
		return;
...
void insert_global_method_decl(No* current) {
	...
	if(current->filho != NULL) {
		...
		while(current != NULL) {
			...
			add_param_to_table(new_table, param_nome, data_type);
